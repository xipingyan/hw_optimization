...
void gemm_gpu_ref(CGEMM_Ref::Ptr gemm_ref_ptr)
{
	...
	std::string kernel_fn = "../05_GEMM/src/gemm_kernel_ref.cl";
	std::string kernel_entry = "gemm_ref";
	...
	std::cout << "  g_enable_half4 = " << g_enable_half4 << std::endl;
	...
	if (g_enable_fp16)
	{
		...
		outputs_gpu = run_gemm_kernel<float>(my_ocl, gemm_ref_ptr, kernel_entry);
