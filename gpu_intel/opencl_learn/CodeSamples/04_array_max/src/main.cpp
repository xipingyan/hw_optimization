...
float run_kernel(CMyTest &my_ocl, std::vector<float> &array_data)
{
	...
	size_t lws = std::min(array_data.size(), g_max_ws_in_one_group[0]);
	...
	cl::Buffer buffer_OUT(my_ocl.get_context(), CL_MEM_READ_WRITE, sizeof(float));

	// Write to device
	my_ocl.get_queue()->enqueueWriteBuffer(buffer_IN_1, CL_TRUE, 0, sizeof(float) * array_data.size(), array_data.data());
	my_ocl.get_queue()->enqueueWriteBuffer(buffer_OUT, CL_TRUE, 0, sizeof(float), &output);
	...
	my_ocl.get_queue()->enqueueReadBuffer(buffer_OUT, CL_TRUE, 0, sizeof(float), &output);
...
void test_array_max_all_group() {
	...
	{
		auto t1 = std::chrono::high_resolution_clock::now();
		max_ocl = run_kernel(my_ocl, array_input);
		auto t2 = std::chrono::high_resolution_clock::now();
		auto diff = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
...
void test_array_max_single_group() {
	...
	auto run_kernel = [&]()
	{
		...
		size_t gws = g_max_ws_in_one_group[0];
		size_t lws = g_max_ws_in_one_group[0];
		// gws = lws = 4;
		size_t group_sz = gws / lws;
		// std::cout << "  == Run kernel:" << std::endl;
		std::cout << "  group_sz = " << group_sz << std::endl;
