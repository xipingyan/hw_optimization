...
void opencl_kernel(ze_device_handle_t hDevice, ze_context_handle_t hContext, ze_module_handle_t &hModule, ze_kernel_handle_t &hKernel, size_t vec_sz) {
	...
	moduleDesc.pInputModule = (uint8_t*)kernel_code.c_str();
	moduleDesc.inputSize = kernel_code.length();
...
bool spirv_kernel(ze_device_handle_t hDevice, ze_context_handle_t hContext, ze_module_handle_t &hModule, ze_kernel_handle_t &hKernel, size_t vec_sz)
{
...
int main()
{
	...
	opencl_kernel(hDevice, hContext, hModule, hKernel, vec_sz);
#else
	spirv_kernel(hDevice, hContext, hModule, hKernel, vec_sz);
#endif
